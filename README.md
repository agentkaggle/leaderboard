...
```text
Kaggle API / leaderboard ZIP / My Submissions
            │
            ▼
...
```dotenv
KAGGLE_API_TOKEN=primary-account-token
...
测试覆盖环境解析、多 token 去重、Unicode team 匹配、官方 Rank、benchmark 排除、四档奖牌阈值、公开目录分组、赛后提交分页与截止时间边界、显式认证、共享限流冷却、严重降级拒绝、部分扫描状态和敏感产物扫描。浏览器验收使用合成 team name，避免把本地目标列表写入测试截图或日志。
